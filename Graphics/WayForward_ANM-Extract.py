...
SpriteName = "20_3466088" # The name of the .ANM, .AN4 or .AN8 file (if UseGBAROM = False) and the sprite tileset PNG file, minus the extensions. Will also be the name of the folder said sprites will extract to. (Example: "20_3466088" will be Shantae's ANM filename when using my QuickBMS script to unpack Shantae Advance: Risky Revolution.)
...
# 3. For a .ANM file, set the UseGBAROM option to False. Change the path for the "SpriteName" entry above to the name of the .ANM file you want to extract (if UseGBAROM = False, otherwise see the section below).
# 4. Extract the tileset graphics for the respective .ANM file (or GBA ROM) using a tile-viewing program such as TiledGGD (https://www.romhacking.net/utilities/646/). Using one of my QuickBMS scripts to split a GBA ROM's filesystem will make additional files with a "_tiles.bin" suffix, which will make that step easier to set up. Pair those tiles up with their respective palettes from either the latter half of a .SCN file or a savestate, and make sure that the extracted PNG matches the ANM file's name (e.g. "Shantae.anm" -> "Shantae.png"), is 128 pixels in width and has a height that encompasses the entire sprite tileset. The "Shantae.png" sample supplied from Shantae Advance will give you an example of what you're aiming for.
# 5. Run the script (no additional command-line parameters needed). If everything's ret-2-go, then resulting sprites can be found in the subfolder named after the "SpriteName" above.

# ROM-file ripping (for experts):
# 3B. Set the UseGBAROM option to True, change the name for the "ROMName" entry to match the GBA ROM you want to try extracting sprites from (eg. "Shantae.gba", it can either be a relative name or a full path), and the "SpriteName" entry to the name of the extracted sprite tiles PNG (see #4 in the previous section).
...
# Troubleshooting:
# If the script throws an error or doesn't export anything, check to make sure that you have the right "ANMFormat" value filled in. If extracting from a GBA ROM instead of a .ANM file, make sure that the "SpriteStart" offset ends in either 0, 4, 8 or C, double-check in a hex editor to make sure you're in the right spot if you need to. Sprite assembly data starts with something similar to "00 00 05 00" with only the third byte being non-zero, and 16 bytes ahead of that should be "00 00 00 00" for the most common ANM formats. Shift it forward or backward by 4 at a time if you need to.
# If the assembled sprites seem to be garbled, make sure that your tileset PNG has a width of 128 pixels (I didn't code it to check for different widths, sorry), and that it is correctly aligned to the beginning of the sprite data. If you're using a "_tiles.bin" exported from one of my GBA-splitting QuickBMS scripts, the starting offset won't need to be adjusted, otherwise you might need to shift forward or backward by a tile, and make sure it doesn't start with a "garbage" tile (empty tiles may be needed for certain sprites).
...
StartX = 0; StartY = 0; HasAltPalette = 0 # ...and initializing the values for tileset PNG reading.
...
SpriteFileStart = (struct.unpack('<L', anmfile.read(4))[0] + SpriteStart) # Relative position from the "SpriteStart" value above (or 0 for an ANM file) where the tile graphics start.
SpriteFileSize = struct.unpack('<L', anmfile.read(4))[0] # Total amount of bytes taken up by the tile graphics.

if not os.path.exists(SpriteName + '.png'):
    if UseGBAROM == False:
        print("Can't find tileset image ('" + SpriteName + ".png'). If this script is working as intended, you should find the tiles at offset " + str(hex(SpriteFileStart)) + " in the '" + SpriteName + ".anm' file.")
    else:
        print("Can't find tileset image ('" + SpriteName + ".png'). If this script is working as intended, you should find the tiles at offset " + str(hex(SpriteFileStart)) + " in the '" + ROMName + "' file (if this doesn't lead to tile data, you likely have the wrong offset for 'SpriteStart').")
    os.system('pause')
    exit()
else:
    sprfile = Image.open(SpriteName + '.png') # Tileset image file needed to build the sprites.
...
if ANMFormat != 1:
    for x in range(FrameTotal):
        FrameOffset[x] = (struct.unpack('<L', anmfile.read(4))[0] + SpriteStart) # Relative position from the "SpriteStart" value above (or 0 for an ANM file) for the sprite's tile assembly information.
        FrameStart[x] = struct.unpack('<L', anmfile.read(4))[0] # Add to the "SpriteFileStart" value above for the start of the sprite's tiles. Not used by the script, but tracked anyway.
...
for x in range(FrameTotal):
    ...
    result_image = Image.new('RGBA', (SpriteWidth, SpriteHeight), (0, 0, 0, 0)) # Initializing the assembled sprite PNG in memory.
    for s in range(PieceCount):
        if PieceSize[s] & 0x8000 == 0:
            TileAdd = (FrameStart[x] // 32) + (TileStart[s]) # Jump to the beginning of the piece's tiles, then shift by the amount.
        else:
            TileAdd = (FrameStart[x] // 64) + (TileStart[s] // 2) # 256-colour tiles need twice the division to work correctly.
        ...
        TileImage = Image.new('RGBA', (TileWidth[s] * 8, TileHeight[s] * 8), (0, 0, 0, 0)) # Initializing the assembled tile chunk in memory.
        for t in range(TileWidth[s] * TileHeight[s]):
            StartX = (TileAdd & 0x0F) * 8 # Maths to determine X position of tile sheet to cut.
            StartY = (((TileAdd & 0xFFF0) // 16) * 8) # More maths to determine Y position of tile sheet to cut.
            if PieceSize[s] & 0x4000 != 0 and HasAltPalette == 0:
                if not os.path.exists(SpriteName + '_1.png'):
                    print("This sprite requires a second tileset with an alternate palette applied to it. Please supply a second image named '" + SpriteName + "_1.png' with its alternate palette applied, and re-run the script.")
                    os.system('pause')
                    exit()
                else:
                    sprfile2 = Image.open(SpriteName + '_1.png') # Second, alternate palette tileset image file needed to build certain sprites.
                    HasAltPalette = 1 # Set this flag so that we don't need to load the file a second time.
            if PieceSize[s] & 0x4000 == 0:
                CropImage = sprfile.crop((StartX, StartY, StartX + 8, StartY + 8)) # Grabbing the right tile.
            else:
                CropImage = sprfile2.crop((StartX, StartY, StartX + 8, StartY + 8)) # Grabbing the right tile from the secondary tileset instead.
            TileImage.paste(CropImage, (TilePasteX * 8, TilePasteY * 8), mask=0) # Pasting the tile in the right spot.
            TileAdd = TileAdd + 1 # Counter go up.
...
if UseGBAROM == True:
    print("The next file should (theoretically) start at around " + str(hex(SpriteFileStart + SpriteFileSize)) + " in " + ROMName + ".")
    os.system('pause')